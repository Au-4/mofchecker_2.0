...
def _guess_underbound_nitrogen_cn2(
  ...
  #  print(angle)
    ...
    if (np.abs(180 - angle) < tolerance) or (  # pylint: disable=no-else-return
        ...
        if (np.abs(mean_dihedral - 180) < tolerance-10) or (np.abs(mean_dihedral - 0) < tolerance-10):
            #print(mean_dihedral)
            # we have to check if the one bond length is smaller than double bonds or two bonds smaller than aromatic partial bonds
            if bond_lengths[0] < 1.4 and bond_lengths[1] < 1.4:
                return False

            if bond_lengths[0] < 1.32 or bond_lengths[1] < 1.32:
                return False           
            return True
        else:
#                print(site_index, angle, bond_lengths, mean_dihedral)
            return True
